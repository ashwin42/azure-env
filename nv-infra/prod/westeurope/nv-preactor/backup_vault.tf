resource "azurerm_recovery_services_vault" "nv-preactor" {
  name                = "nv-preactor-recovery-vault"
...
resource "azurerm_recovery_services_protection_policy_vm" "daily" {
  name                = "daily"
  resource_group_name = var.resource_group_name
  recovery_vault_name = azurerm_recovery_services_vault.nv-preactor.name
