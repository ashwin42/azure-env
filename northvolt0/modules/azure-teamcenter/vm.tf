...
resource "azurerm_virtual_machine" "teamcenter" {
  ...
  network_interface_ids            = [azurerm_network_interface.teamcenter_network_interface[count.index].id]
...
resource "azurerm_virtual_machine_data_disk_attachment" "teamcenter_data" {
  count              = var.teamcenter_server_count
  managed_disk_id    = azurerm_managed_disk.teamcenter_data[count.index].id
  virtual_machine_id = azurerm_virtual_machine.teamcenter[count.index].id
  lun                = "5"
  caching            = "ReadWrite"
}
...
resource "azurerm_recovery_services_protected_vm" "teamcenter" {
  ...
  source_vm_id        = azurerm_virtual_machine.teamcenter[count.index].id
...
resource "azurerm_virtual_machine_extension" "teamcenter" {
  ...
  location             = var.location
  resource_group_name  = var.resource_group_name
  virtual_machine_name = azurerm_virtual_machine.teamcenter[count.index].name
...
resource "azurerm_virtual_machine" "tc_gpu" {
  ...
  location                         = var.location
  resource_group_name              = var.resource_group_name
  network_interface_ids            = [azurerm_network_interface.tc_gpu_network_interface[0].id]
...
resource "azurerm_recovery_services_protected_vm" "tc_gpu" {
  ...
  resource_group_name = var.resource_group_name
  recovery_vault_name = azurerm_recovery_services_vault.teamcenter.name
  source_vm_id        = azurerm_virtual_machine.tc_gpu[0].id
  backup_policy_id    = azurerm_recovery_services_protection_policy_vm.daily.id
}
